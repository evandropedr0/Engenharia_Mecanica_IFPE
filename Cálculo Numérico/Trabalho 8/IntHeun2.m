function [ I,X,Y ] = IntHeun2(eq_dif,a,b,h,Cix,Ciy,it)
...
%Y: vetor Y (saída)
    ...
    for i = 2:length(X)
        %cálculo do preditor
        Y(i,1) = Y(i-1,1) + subs(eq_dif,{x,y},{X(i-1,1),Y(i-1,1)})*h;
        %laço que faz a quantidade de iterações necessárias a cada ponto
        for j = 1:1:it
            %corretor
            Y(i,1) = Y(i-1,1)+((subs(eq_dif,{x,y},{X(i-1,1),Y(i-1,1)})+subs(eq_dif,{x,y},{X(i-1,1)+h,Y(i,1)}))*h)/2;
        end
    end
