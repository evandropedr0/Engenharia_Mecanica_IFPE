function [ I,X,Y ] = IntPtMedio2(eq_dif,a,b,h,Cix,Ciy)
%Realiza integração numérica de uma equação diferencial pelo método do
%Ponto Médio.
...
%Y: vetor Y (saída)
    ...
    for i = 2:length(X)
        %cálculo do y médio
        Ym = Y(i-1,1) + (subs(eq_dif,{x,y},{X(i-1,1),Y(i-1,1)})*h/2);
        %cálculo no ponto médio
        Y(i,1) = Y(i-1,1) + subs(eq_dif,{x,y}, {X(i-1,1)+h/2, Ym})*h;
    end
